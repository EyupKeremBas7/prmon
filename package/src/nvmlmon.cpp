...
void nvmlmon::update_stats(const std::vector<pid_t>& pids,
  ...
  for (unsigned int gpu_idx{0}; gpu_idx < ngpus; ++gpu_idx) {
    ...
    result = nvmlDeviceGetProcessUtilization(device, utilization.data(),
                                             &util_count, last_seen_timestamp);
    ...
    for (unsigned int target_pid : pids) {
      for (unsigned int i{0}; i < util_count; ++i) {
        if (utilization[i].pid == target_pid) {
          nvml_stats_update["gpusmpct"] += utilization[i].smUtil;
          nvml_stats_update["gpumempct"] += utilization[i].memUtil;
          gpu_is_active = true;
          break;
        }
      }
      for (unsigned int i{0}; i < mem_count; ++i) {
        if (memory_info[i].pid == target_pid) {
          nvml_stats_update["gpufbmem"] +=
              (memory_info[i].usedGpuMemory / BYTES_PER_KB);
          gpu_is_active = true;
          break;
        }
      }
